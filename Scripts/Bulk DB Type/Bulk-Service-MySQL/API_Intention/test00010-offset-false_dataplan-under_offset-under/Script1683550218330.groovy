...
WS.sendRequest(findTestObject('MYSQL_API_int_without_Offset', [('DataPlan') : 100]))

WS.verifyElementPropertyValue(responseAPI, 'response', 'mysql-post-bulk-service triggered Successfully')

WS.delay(30)

FinalEntries1 = CustomKeywords.'mySQL.mySqlFunction.CountTableEntries'(GlobalVariable.MySqlURI, GlobalVariable.MySqlUserName, 
    GlobalVariable.MySqlPassword, 'smdb.TimeStamp')

FinalcountEntries1 = CustomKeywords.'mongoDatabaseOperations.MongoConnect.countDBEntries'(GlobalVariable.Mongo_URL, GlobalVariable.MongoDBName, 
    'NOTIFICATION_EMAIL_HISTORY')
...
FinalEntries2 = CustomKeywords.'mySQL.mySqlFunction.CountTableEntries'(GlobalVariable.MySqlURI, GlobalVariable.MySqlUserName, 
    GlobalVariable.MySqlPassword, 'smdb.TimeStamp')

FinalcountEntries2 = CustomKeywords.'mongoDatabaseOperations.MongoConnect.countDBEntries'(GlobalVariable.Mongo_URL, GlobalVariable.MongoDBName, 
    'NOTIFICATION_EMAIL_HISTORY')

WS.verifyEqual(FinalEntries1+1, FinalEntries2)

WS.verifyEqual(FinalcountEntries1+1, FinalcountEntries2)