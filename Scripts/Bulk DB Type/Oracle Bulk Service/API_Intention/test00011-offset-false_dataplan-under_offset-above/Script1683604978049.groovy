...
response = WS.sendRequest(findTestObject('bulk_master_api/oracle/oracel_Api_int', [('ServiceName') : GlobalVariable.oracal_api_int_rest[
            0], ('BulkID') : GlobalVariable.oracal_api_int_rest[1], ('StopLimit') : 10001, ('OffsetEnable') : false]))
...
WS.verifyElementPropertyValue(response123, GlobalVariable.oracal_index[2], 50, FailureHandling.STOP_ON_FAILURE)

WS.verifyElementPropertyValue(response123, GlobalVariable.oracal_index[4], false, FailureHandling.STOP_ON_FAILURE)
...
responseAPI = WS.sendRequest(findTestObject('Oracel_Api_intmention_withoffset', [('id') : 100]))
...
////////////////////////////
WS.sendRequest(findTestObject('Oracel_Api_intmention_withoutoffset', [('id') : 100]))

WS.verifyElementPropertyValue(responseAPI, 'response', 'oracledb-test triggered Successfully')

FinalEntries123 = CustomKeywords.'mySQL.mySqlFunction.CountTableEntries'(GlobalVariable.MySqlURI, GlobalVariable.MySqlUserName, 
    GlobalVariable.MySqlPassword, 'smdb.TimeStamp')
...
FinalEntries1234 = CustomKeywords.'mySQL.mySqlFunction.CountTableEntries'(GlobalVariable.MySqlURI, GlobalVariable.MySqlUserName, 
    GlobalVariable.MySqlPassword, 'smdb.TimeStamp')
