...
response = WS.sendRequest(findTestObject('bulk_master_api/file/notification-bulk-service', [('ServiceName') : GlobalVariable.Notification_BulkProcess
            , ('BulkID') : GlobalVariable.Notification_BulkID, ('MandatoryFields') : '0,1,2,3,4,5,6',('SkipRowCount') : 0,('CompressFiles') : false]))
...
CustomKeywords.'sftp_keyword.refactor_sftp.deleteAllRecords'(GlobalVariable.Sftp_inform[0], GlobalVariable.Sftp_inform[1], 
    GlobalVariable.Sftp_inform[2], GlobalVariable.Notification_bulks_services_host, GlobalVariable.Sftp_inform[3], '/bulk-store/ErrorFiles')
...
uploadresponse = WS.sendRequestAndVerify(findTestObject('FileUpload', [('path') : 'files/MandatoryFields-errorfolder-allthemandatory.csv'
...
WS.verifyElementPropertyValue(uploadresponse, 'uploadDetails[0].FileName[0]', 'MandatoryFields-errorfolder-allthemandatory.csv')
...
WS.verifyEqual(countFiles1, countFiles2)

WS.verifyEqual(countEntries1, countEntries2)
