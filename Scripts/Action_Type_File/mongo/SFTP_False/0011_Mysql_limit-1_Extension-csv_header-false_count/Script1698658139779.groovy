...
response = WS.sendRequest(findTestObject('bulk_master_api/mongoDB/Action_type-mongo', [('ServiceName') : GlobalVariable.Sync_BulkProcess, ('BulkID') : GlobalVariable.Sync_BulkID
	...
	, ('pvc') : false, ('sftp') : false, ('enrichEnable') : false, ('Limit') : '1']))
...
filecount = CustomKeywords.'basicOperations.RandomStringGenerator.filecount'(5, 1)
...
countmysql = CustomKeywords.'mongoDatabaseOperations.MongoConnect.countDBEntries'(GlobalVariable.Mongo_URL, 
    GlobalVariable.MongoDBName, 'MONGO_DB_BULK_TEST')
