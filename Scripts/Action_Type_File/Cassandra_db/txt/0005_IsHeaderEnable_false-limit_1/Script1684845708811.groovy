...
response = WS.sendRequest(findTestObject('bulk_master_api/Cassandra-db/Actiontype-file', [('ServiceName') : GlobalVariable.CassandraDB_actionType_file[
            ...
            0], ('IsHeaderEnable') : GlobalVariable.Offset_false, ('SftpEnable') : GlobalVariable.Offset_false, ('Notify') : GlobalVariable.Offset_false
 ...
 * CassandraDB_actionType_file_indexing[2],GlobalVariable.FileExtension[0])
 * 
 * WS.verifyElementPropertyValue(response1,GlobalVariable.
...
'verifying all the records get deleteded'
WS.verifyEqual(countSFTP1, 0)

'triggering bulk services'
response2 = WS.sendRequest(findTestObject('Cassandra_triggerapi', [('bulkProcessName') : GlobalVariable.CassandraDB_actionType_file[
            0], ('URL') : GlobalVariable.URL, ('valeid1') : GlobalVariable.valeid[1], ('valeid') : GlobalVariable.valeid[
            0]]))

'verfying bulk get triggered successfully'
WS.verifyElementPropertyValue(response2, 'response', 'cache-post-bulk-service triggered Successfully')

'verifying count after the bulk services'
...
'verifying count is verifying get sucessfull in base SFTP'
WS.verifyEqual(countSFTP2, ((GlobalVariable.valeid[1]) - (GlobalVariable.valeid[0])) + 1)

'printing all the all the records get printed on based sftp'
headers = CustomKeywords.'sftp_keyword.refactor_sftp.readAllLines'(GlobalVariable.Sftp_inform[0], GlobalVariable.Sftp_inform[
    1], GlobalVariable.Sftp_inform[2], GlobalVariable.cache_post_service_host, GlobalVariable.Sftp_inform[3], '/bulk-store')

println(headers)

'converting header string to header toString method'
...
header_present = CustomKeywords.'sftp_keyword.refactor_sftp.Header_is_present'(GlobalVariable.Sftp_inform[0], GlobalVariable.Sftp_inform[
    1], GlobalVariable.Sftp_inform[2], GlobalVariable.cache_post_service_host, GlobalVariable.Sftp_inform[3], '/bulk-store', 
    'usage_units,usage_discount,id,testdataone,holiday_date')
